...
if (argv_len == 1) or ("-h" in sys.argv) or ("-H" in sys.argv) or ("--help" in sys.argv):
    print("Usage:          snphub stop <CONTAINER ID>")
    print("Description:    stop a specific container")
    print("")
else:
    if argv_len>2:
        print("Wrong parameter. Enter \"snphub stop -h\" for help.")
    else:
        container_id = str(sys.argv[1])
        try:
            out_bytes = subprocess.call(['docker', 'stop', container_id])
        except:
            print("")
