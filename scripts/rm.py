...
if (argv_len == 1) or ("-h" in sys.argv) or ("-H" in sys.argv) or ("--help" in sys.argv):
    print("Usage:          snphub rm <CONTAINER ID>")
    print("Description:    delete a specific container")
    print("")
else:
    if argv_len>2:
        print("Wrong parameter. Enter \"snphub rm -h\" for help.")
    else:
        container_id = str(sys.argv[1])
        try:
            out_bytes = subprocess.call(['docker', 'rm', container_id])
        except:
            print("")
