...
if (argv_len == 1) or ("-h" in sys.argv) or ("-H" in sys.argv) or ("--help" in sys.argv):
    print("Usage:          snphub start <CONTAINER ID>")
    print("Description:    start a specific container")
    print("")
else:
    if argv_len>2:
        print("Wrong parameter. Enter \"snphub start -h\" for help.")
    else:
        container_id = str(sys.argv[1])
        try:
            out_bytes = subprocess.call(['docker', 'start', container_id])
        except:
            print("")
